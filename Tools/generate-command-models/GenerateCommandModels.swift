...
struct GenerateCommandModels: AsyncParsableCommand {
    ...
    func run() async throws {
        let swiftlyCoreDir = fs.cwd / "Sources/SwiftlyCore"

        let cmdHelpFiles = (try await fs.ls(atPath: swiftlyCoreDir)).filter { $0.hasSuffix(".json") && !$0.hasSuffix("-ext.json") }.sorted()
        ...
        for cmdHelp in cmdHelpFiles {
            let data = try Data(contentsOf: swiftlyCoreDir / cmdHelp)
            let toolInfoThin = try JSONDecoder().decode(ToolInfoHeader.self, from: data)
            guard toolInfoThin.serializationVersion == 0 else {
                fatalError("Unsupported serialization version in \(swiftlyCoreDir / cmdHelp)")
            }

            let toolInfo = try JSONDecoder().decode(ToolInfoV0.self, from: data)

            let cmdExtData = try? Data(contentsOf: swiftlyCoreDir / cmdHelp.replacingOccurrences(of: ".json", with: "-ext.json"))
        ...
        try await allCmds.write(to: swiftlyCoreDir / "CommandsNew.swift-new", atomically: true)

        let data = self.sha256sumjson.data(using: .utf8)!

        let toolInfoThin = try JSONDecoder().decode(ToolInfoHeader.self, from: data)
        guard toolInfoThin.serializationVersion == 0 else {
            fatalError("Unsupported serialization version")
        }

        let toolInfo: ToolInfoV0 = try JSONDecoder().decode(ToolInfoV0.self, from: data)

        let extdata = self.sha256sumextension.data(using: .utf8)!

        let cmdExt: CommandExtensions = try JSONDecoder().decode(CommandExtensions.self, from: extdata)

        let top = toolInfo.command

        print("\(self.asCommand(top, cmdExt))")
    }
